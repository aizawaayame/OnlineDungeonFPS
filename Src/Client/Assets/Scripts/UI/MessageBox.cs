...
namespace UI
{
    public class MessageBox
    {
        ...
        public static UIMessageBox Show(string message, string title="", MessageBoxType type = MessageBoxType.Information, string btnOK = "", string btnCancel = "")
        {
            if(cacheObject==null)
            {
                cacheObject = Assets.Resloader.Load<Object>("UI/UIMessageBox");
            }

            GameObject obj = (GameObject)GameObject.Instantiate(cacheObject);
            UIMessageBox msgbox = obj.GetComponent<UIMessageBox>();
