...
namespace Services
{
    class UserService : Singleton<UserService>, IDisposable
    {
        ...
        {
            Debug.LogFormat("OnUserCreateCharacter:{0} [{1}],Count{2}", response.Result, response.Errormsg,response.Characters.Count);
        ...
        {
            Debug.LogFormat("SendGameEnter:{0}", idx);
            NetMessage message = new NetMessage();
            message.Request = new NetMessageRequest();
        ...
        {
            Debug.LogFormat("收到进入游戏：{0}", response.Result);
            if (response.Result == Result.Success)
            {
